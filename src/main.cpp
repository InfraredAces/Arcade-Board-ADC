...
int main() {
    ...
    gpio_init(GPIO_TEST_00);
    gpio_set_dir(GPIO_TEST_00, GPIO_OUT);
    gpio_init(GPIO_TEST_01);
    gpio_set_dir(GPIO_TEST_01, GPIO_OUT);
    ...
    while(true) {
        for(size_t i = 0; i < CD74HC4067SM96_NUM_CHANNELS; i++) {
            ...
            // if(MUX_01.readChannel(i) > ADC_TRIGGER_THRESHOLD) {
            //     gpio_put(GPIO_TEST_01, true);
            // } else {
            //     gpio_put(GPIO_TEST_01, false);
            // }
